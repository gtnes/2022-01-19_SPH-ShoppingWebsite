  ...
  <div class="clearfix selector">
    <div class="type-wrap logo">
      ...
      <div class="value logos">
        <ul class="logo-list">
          <li v-for="(trademark, index) in trademarkList" :key="index" @click="tradeMatHandler(trademark)">{{trademark.tmName}}</li>
        </ul>
      </div>
      <div class="ext">
  ...
  export default {
    ...
    methods:{
      tradeMatHandler(trademark){
        // 触发父组件的自定义事件 tradmarkInfo
        this.$emit('tradmarkInfo', trademark) //(事件名称, 携带参数)
      },

      // 平台售卖属性值的事件
      attrInfo(attr, attrValue){
        this.$emit('attrInfo', attr, attrValue) //(事件名称, 携带参数)
      }
    }
  }
