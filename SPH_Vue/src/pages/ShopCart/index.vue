      ...
      <div class="cart-body">
        ...
        >
          ...
          <li class="cart-list-con2">
            <img :src="cart.imgUrl" @click="openDetail(cart.skuId)" />
            <div class="item-msg" @click="openDetail(cart.skuId)">
              {{ cart.skuName }}
            </div>
          </li>
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="deleteShopCart(cart)">删除</a>
            <br />
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    // 修改购物车产品个数(使用throttle节流，防止点击过快发生错误)
      ...
      switch (type) {
        ...
        case "change":
          if (isNaN(disNum) || disNum < 1) {
            // 检测输入是否为数字或为负数，disNum为0，商品数量不增加和不减少
            disNum = 0;
          } else {
            // parseInt将小数转换为整数
    ...
    async deleteShopCart(cart) {
      try {
        await this.$store.dispatch("deleteShopCartBySkuId", cart.skuId);
        this.getData();
      } catch (error) {
        alert(error.message);
      }
  ...
  computed: {
    ...
    // 全选勾选
    isAllCheck() {
      // every方法变量数组，满足条件返回true，不满足条件返回false
